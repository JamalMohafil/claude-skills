...
- **⭐ A record with fields renders as FIELDS.** Chips for taxonomy, badges for state, a pull-quote for the hook, a link for the source — never a record stringified into a paragraph. The stores here are deliberately structured (ideas, radar runs, pieces, chats); a view that flattens them back into prose throws away the whole point and cannot be scanned. If you are joining fields with ` — ` or reaching for `JSON.stringify`, the view is wrong.
- **⭐ Data specified ≠ screen built.** Each reference describes a store and then how to *arrange* what it shows; it is easy to read both and never build the screen in between. The views that get skipped this way, every time: the **post detail window** (ref 6), the **Produce views** (ref 3), and the **Radar view** (ref 5b). Check those three by name before you call a build done.
